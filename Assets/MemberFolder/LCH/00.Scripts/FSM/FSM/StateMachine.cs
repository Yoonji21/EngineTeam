...
public class StateMachine : MonoBehaviour
{
    ...
    public StateMachine(EntityStatesSO entityFSM, Entity owner)
    {
        ...
        foreach (StateSO state in entityFSM.states)
        {
            ...
            {
                Debug.LogError($"<color=red>{state.className}</color> " +
                    $"loading error, Message : {ex.Message}");
            }
        }
    }
    ...
    public void UpdateStaetMachine()
    {
        currentState.Update();
    }
    ...
    public void ChangeState(string newState)
    {
        ...
        Debug.Assert(nextState != null, $"State : {newState} no found");
